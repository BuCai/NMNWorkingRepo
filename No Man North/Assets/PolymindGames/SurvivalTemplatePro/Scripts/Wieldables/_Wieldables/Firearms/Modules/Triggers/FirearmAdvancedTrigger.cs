...
{
    [AddComponentMenu("Wieldables/Firearms/Triggers/Advanced Trigger")]
	public class FirearmAdvancedTrigger : FirearmTriggerBehaviour
	{
		...
		[SerializeField, ShowIf("m_Firemode", (int)FireMode.SemiAuto), Range(0f, 10f)]
		[Tooltip("The minimum time that can pass between consecutive shots.")]
		private float m_PressCooldown = 0.22f;

		[SerializeField, ShowIf("m_Firemode", (int)FireMode.Burst), Range(0, 100)]
		[Tooltip("How many shots will the gun fire when in Burst-mode.")]
		private int m_BurstLength = 3;

		[SerializeField, ShowIf("m_Firemode", (int)FireMode.Burst), Range(0f, 100f)]
		...
		[SerializeField, ShowIf("m_Firemode", (int)FireMode.FullAuto), Range(0, 10000)]
		...
		protected override void TapTrigger()
		{
			...
			if (Firearm.Reloader.IsReloading || Firearm.Reloader.IsMagazineEmpty)
				Shoot(1f);
			else if (m_Firemode == FireMode.SemiAuto)
				Shoot(1f);
