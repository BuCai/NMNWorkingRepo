...
{
    [AddComponentMenu("Wieldables/Firearms/Shooters/Hitscan Shooter")]
    public class FirearmHitscanShooter : FirearmShooterBehaviour
    {
        ...
        public override void Shoot(float value)
        {
            ...
            // Events
            Firearm.EventHandler.TriggerAction(m_ShootEvent, 1f);
        }

        private void DoHitscan(ICharacter user, Ray ray)
        {
            if (Physics.Raycast(ray, out RaycastHit hitInfo, m_MaxDistance, m_RayMask, m_TriggerInteraction))
            {
                ...
                if (hitInfo.collider.TryGetComponent(out IDamageReceiver receiver))
                    receiver.HandleDamage(new DamageInfo(-damage, m_DamageType, hitInfo.point, ray.direction, impulse, user));

                SurfaceManager.SpawnEffect(hitInfo, SurfaceEffects.BulletHit, 1f, isDynamicObject);

#if UNITY_EDITOR
                if (m_DebugHitscanRay)
                    Debug.DrawLine(ray.origin, hitInfo.point, Color.red, 5f);
#endif
            }
        }
    }
