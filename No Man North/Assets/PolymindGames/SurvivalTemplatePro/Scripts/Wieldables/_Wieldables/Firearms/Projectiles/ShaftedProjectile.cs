...
namespace SurvivalTemplatePro
{
	...
	public class ShaftedProjectile : MonoBehaviour, IProjectile
	{
		...
		public float DamageMod { get; set; } = 1f;

		[Title("Detection")]

		[SerializeField]
		private LayerMask m_DetectionMask = (LayerMask)172545;
		...
		public void AttachItem(IItem itemToAttach) 
		{
			if (m_Pickup != null)
		...
		public void CheckForSurfaces(Vector3 position, Vector3 direction)
		{
			...
			if (Physics.Raycast(position, direction, out RaycastHit hitInfo, 2f, m_DetectionMask, m_TriggerInteraction) && !m_Launcher.HasCollider(hitInfo.collider))
			{
				...
				if (hitInfo.collider.TryGetComponent(out IDamageReceiver receiver))
				{
					...
					float damage = m_MaxDamage * damageMod * DamageMod;
				...
				HandleSurfacePenetration(hitInfo);

				// Disable the trail if available.
				if (m_Trail != null)
		...
		private void HandleSurfacePenetration(RaycastHit hitInfo)
		{
		...
		{
			if (m_Launched && !m_HasPenetratedSurface)
				CheckForSurfaces(transform.position, transform.forward);

			if (m_PenetratedSurface)
				MatchWithPenetratedSurface();
		}
		...
		private void MatchWithPenetratedSurface()
		{
			...
			{
				var position = m_PenetratedSurface.transform.position + m_PenetratedSurface.transform.TransformVector(m_PenetrationPositionOffset);
				var rotation = m_PenetratedSurface.transform.rotation * m_PenetrationRotationOffset;
		...
		private IEnumerator C_DoTwang()
		{
			...
			var twangPosition = transform.position + m_TwangSettings.MovementPivot.LocalToWorld(transform);
			var twangRotation = transform.rotation;
