    ...
    public class WieldableRigidbodyEffect : WieldableObjectSpawnEffect
	{
        ...
        protected override PoolableObject SpawnEffect()
        {
			...
			if (obj.gameObject.TryGetComponent(out Rigidbody objRb))
			{
				...
				Transform rotationParent = m_VelocityAlignment == VelocityAlignment.Local ? transform : m_Character.ViewTransform;
				Vector3 velocity = (m_Character.GetModule<ICharacterMotor>().Velocity * 0.1f) + rotationParent.TransformVector(Vector3.forward * m_Speed + velocityJitter);
