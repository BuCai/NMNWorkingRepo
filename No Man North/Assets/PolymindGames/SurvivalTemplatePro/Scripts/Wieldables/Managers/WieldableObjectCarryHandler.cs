...
{
    [HelpURL("https://polymindgames.gitbook.io/welcome-to-gitbook/qgUktTCVlUDA7CAODZfe/player/modules-and-behaviours/wieldable#wieldable-object-carry-handler-behaviour")]
    public class WieldableObjectCarryHandler : CharacterBehaviour
    {
        ...
        private void OnObjectCarryStart()
        {
            ...
            if (carriedObject != null)
            {
                ...
                if (m_Carriables.TryGetValue(carriedObject, out var handler))
                {
                    ...
                    m_WieldablesController.TryEquipWieldable(handler.AttachedWieldable, 1.3f);
        ...
        private void AddCarriableToDictionary(CarriableDefinition carriableDefinition)
        {
            ...
            var visualHandler = spawnedWieldable.gameObject.GetComponentsInChildren<IVisualCarriablesHandler>(true)[0];
