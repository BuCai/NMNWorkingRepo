...
namespace SurvivalTemplatePro.UISystem
{
    public class BuildMaterialsUI : PlayerUIBehaviour
    {
        ...
        {
            Vector2 screenPositionOfPreview = m_Camera.WorldToScreenPoint(preview.PreviewCenter);
        ...
        private void OnStructureInViewChanged(BuildablePreview preview)
        {
            if (m_StructureInView != null)
            {
                m_StructureInView.onMaterialAdded -= SetBuildRequirementsInfo;
                m_StructureInView.onBuildCompleted -= OnBuildCompleted;
            }
        ...
        private void SetBuildRequirementsInfo(BuildablePreview preview)
        {
            for (int i = 0; i < m_MaterialDisplayers.Length; i++)
            {
                ...
                var buildRequirements = preview.GetAllBuildRequirements();
