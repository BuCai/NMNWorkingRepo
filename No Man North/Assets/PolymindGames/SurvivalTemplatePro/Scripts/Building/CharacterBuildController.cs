...
namespace SurvivalTemplatePro.BuildingSystem
{
    ...
    public class CharacterBuildController : CharacterBehaviour, IBuildingController
    {
        ...
        {
            if (placeable == m_PlaceablePrefab)
                return;

            ResetInfo();

            if (placeable != null)
                CreatePlaceable(placeable);
        ...
        public void PlaceObject()
        {
            ...
            // Play place effects.
            CameraShakeHandler.DoShake(m_CameraShake, 1f);
        ...
        private Socket FindSocket(Buildable buildable)
        {
            ...
            for (int i = 0; i < proximityColliders.Length; i++)
            {
                ...
                {
                    Ray viewRay = new Ray(Character.ViewTransform.position, Character.ViewTransform.forward);
        ...
        private void PlaceObject(Placeable placeable)
        {
            ...
            if (buildable == null)
            {
                ...
                FreeBuildablePreview preview = placeableInstance.AddComponent<FreeBuildablePreview>();
            ...
            else
            {
                if (m_Socket != null)
                {
                    ...
                    StructurePreview preview = structure.gameObject.GetComponent<StructurePreview>();
                    preview.EnablePreview();
                    ...
                    structure.AddPart(part, true);

                    if (structure.Buildables.Count > 1)
                        OccupySurroundingSockets(part);
                }
                else
                {
                    ...
                    if (isChildOfStructure)
                    {
                        ...
                        StructurePreview preview = structure.gameObject.AddComponent<StructurePreview>();
                    ...
                    else
                    {
                        ...
                        FreeBuildablePreview preview = simpleBuildable.AddComponent<FreeBuildablePreview>();
