...
namespace SurvivalTemplatePro.BuildingSystem
{
	...
	public class Door : Interactable, ISaveableComponent
	{
		[SerializeField]
        ...
        private void Awake()
        {
			m_ClosedRotation = m_Model.localRotation;
			m_AudioSource = GetComponent<AudioSource>();
		}

        private IEnumerator C_DoAnimation(ICharacter character)
		{
			bool open = Quaternion.Angle(m_ClosedRotation, m_Model.localRotation) > 0.5f;
			Quaternion targetRotation = m_ClosedRotation;

			if (!open)
			{
				bool characterIsInFront = Vector3.Dot(character.transform.forward, m_Model.forward) > 0f;
				Vector3 modelEulerAngles = m_Model.localEulerAngles;
				targetRotation = Quaternion.Euler(characterIsInFront ? modelEulerAngles + m_OpenRotation : modelEulerAngles - m_OpenRotation);
			}

			bool shouldMove = Quaternion.Angle(targetRotation, m_Model.localRotation) > 0.5f;
			...
			if (shouldMove)
			{
				...
				{
					m_Model.localRotation = Quaternion.Lerp(m_Model.localRotation, targetRotation, Time.deltaTime * m_AnimationSpeed);
					yield return null;
				}
