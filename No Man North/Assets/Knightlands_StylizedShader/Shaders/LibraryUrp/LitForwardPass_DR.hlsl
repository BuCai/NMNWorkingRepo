...
/* LitPassVertex(Attributes input)
{
    Varyings output;
...
half4 StylizedPassFragment(Varyings input) : SV_Target
{
    ...
    color.rgb = lerp(surfaceData.albedo, UniversalFragmentPBR(inputData, surfaceData), _MetallicSwitch);
    ...
    half4 returnCol = color;
  
    return float4(returnCol.rgb * UniversalFragment_DSTRM(inputData, surfaceData.albedo, surfaceData.emission, surfaceData.alpha), returnCol.a * _ColorMultiplier.a);
