...
half3 LightingPhysicallyBased_DSTRM(Light light, half3 normalWS, half3 viewDirectionWS, half3 positionWS) {
    ...
    half4 shadowGradient = SAMPLE_TEXTURE2D(_GradientRamp, sampler_GradientRamp, gradient_uv);
    half4 highlightGradient = SAMPLE_TEXTURE2D(_GradientRampHighLight, sampler_GradientRampHighLight, gradient_uv);
    half4 c = (shadowGradient + highlightGradient) * _ColorMultiplier;
    c.rgb *= light.color;
    ...
    c = lerp(c, c+_FlatSpecularColor, specularTransition);
    ...
    shadowAttenuation *= smoothstep(_ShadowSoftness, 0, -min(NdotL, 0));
//#if defined(DR_LIGHT_ATTENUATION)
    ...
    c.rgb *= lerp(shaded *c.rgb, c.rgb, shadowAttenuation);
#else
